...
void Int::ModInvK1() {
    // 1) exp = n - 2, где n = *_O
    Int exp(*_O);
    exp.Sub(&_ONE);     // exp = n - 1
    exp.Sub(&_ONE);     // exp = n - 2

    // 2) стандартная бинарная экспонентация через ModMulK1order
    Int result((int64_t)1);    // результат = 1
    Int base(*this);           // base = this

    int bits = exp.GetBitLength();
    for (int i = bits - 1; i >= 0; --i) {
        // result = result^2 mod n
        result.ModMulK1order(&result);
        // если бит = 1, множим на base
        if (exp.GetBit(i)) {
            result.ModMulK1order(&base);
        }
    }

    // 3) записываем результат обратно в this
    Set(&result);
}
